...
# Pioneer Autonomous Navigation – ROS 2 + Gazebo

## Overview

This project simulates a Pioneer robot in Gazebo and uses ROS 2 to enable autonomous navigation. The robot is equipped with LiDAR, camera, IMU, and odometry, and uses a custom DistBug-style controller to navigate to waypoints while avoiding obstacles.
...
Project 1/
├── robots/                 # URDF + meshes  
├── basic_urdf.sdf         # Gazebo world  
├── ros2_ws/               # ROS 2 workspace  
│   ├── src/  
│   │   └── pioneer_nav/  
│   │       └── distbug_controller.py  

---

## Build Instructions

cd "/mnt/c/Users/Lithi/Desktop/AUTO4408/Project 1/ros2_ws"  
...
We bridge Gazebo topics into ROS 2 using `ros_gz_bridge`, allowing us to visualise and use sensor data.
...
0 0 0 0 0 0 \
base_link pioneer/base_link/laser
```
...
# Terminal 6 - World Pose
```sh
source /opt/ros/jazzy/setup.bash
ros2 run ros_gz_bridge parameter_bridge \
"/world/pioneer_world/dynamic_pose/info@geometry_msgs/msg/PoseArray@gz.msgs.Pose_V"
```
...
# Terminal 10 — Controller
```sh
source /opt/ros/jazzy/setup.bash
source "/mnt/c/Users/Lithi/Desktop/AUTO4408/Project 1/ros2_ws/install/setup.bash"

ros2 run pioneer_nav distbug_controller --ros-args \
-p world_pose_topic:=/world/pioneer_world/dynamic_pose/info \
-p robot_name:=pioneer
```
# Rebuild

```sh
cd "/mnt/c/Users/Lithi/Desktop/AUTO4408/Project 1/ros2_ws"
colcon build --packages-select pioneer_nav
...
The robot uses a DistBug-style algorithm for navigation.

Behaviour:
1. Move toward the goal  
2. Detect obstacles using LiDAR  
3. Follow obstacle boundary when blocked  
4. Resume goal direction when path is clear  
5. Stop when waypoint is reached  
...
The simulation world was modified to:
- Resemble an oval field  
- Include obstacles (cones, trees, structures)  

This allows testing of obstacle avoidance and navigation.
...
- Source ROS 2 in every terminal before running commands  
- Source the workspace before running the controller  
- TF must be correct for RViz to work  