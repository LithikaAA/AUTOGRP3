...
class ControlNode(Node):
    def __init__(self):
        ...
        self.state_pub = self.create_publisher(String, '/robot_state',  10)  # GUI state badge
        self.pose_pub  = self.create_publisher(Pose,   '/robot/pose',   10)  # GUI map arrow
        self.arena_pub = self.create_publisher(String, '/arena_status', 10)  # GUI arena panel
    ...
    def publish_arena_status(self, rel_x, rel_y):
        ...
        # Publish as JSON for the GUI arena panel
        msg = String()
        msg.data = json.dumps({
    ...
    def control_loop(self):
        with self.mutex:
            ...
            self.publish_arena_status(rel_x, rel_y)  # replaces log_arena_status
            ...
            if not (-MAP_HALF_SIZE <= rel_x <= MAP_HALF_SIZE and -MAP_HALF_SIZE <= rel_y <= MAP_HALF_SIZE) and self.auto_state != AUTO_STATE.RETURN_TO_CENTER:
                ...
                self.target_yaw = None
                self.publish_twist(0.0, 0.0)
                self.publish_state('RETURN_TO_CENTER')
    ...
    def handle_return_to_center(self):
        ...
        target_yaw_to_center_norm = (math.degrees(math.atan2(vec_y, vec_x)) + 360) % 360
    ...
    def _calculate_general_inward_direction(self):
        rel_x, rel_y = self.relative_position()
        return (math.degrees(math.atan2(MAP_CENTER[1] - rel_y, MAP_CENTER[0] - rel_x)) + 360) % 360
