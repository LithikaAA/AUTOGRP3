#!/usr/bin/env python3
"""
unified_detector_node.py — ORIGINAL BRIGHTNESS VERSION
-------------------------------------------------------
Uses brightness thresholding + Canny edges to find white paper.
Works reliably indoors (100% accuracy tested).
For outdoor use switch to the HSV version.

Default parameters (daylight indoors):
...
class UnifiedDetectorNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("min_paper_brightness",   150)
        self.declare_parameter("min_dark_ratio",         0.02)
        self.declare_parameter("max_dark_ratio",         0.5)
        ...
        self.get_logger().info(
            f"Original detector: brightness_threshold={self.bright_thresh}  "
            f"min_paper_brightness={self.min_paper_bright}  "
            f"min_dark_ratio={self.min_dark_ratio}"
        )
    ...
    # ── Sign region detection — original brightness + edge ────────────────────

    def find_sign_region(self, gray: np.ndarray):
        """
        Find white A4 paper using brightness thresholding + Canny edges.
        Works reliably indoors. For outdoor use see HSV version.
        """
        h, w   = gray.shape
        img_cx = w // 2
        img_cy = h // 2
    ...
    # ── Letter extraction and classification ──────────────────────────────────
